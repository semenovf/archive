...
void _cwt_process_keyboard(void)
{
	if( bioskey(_KEYBRD_READY) ) { /* is equivalent to kbhit() but last resides in conio.h header */
		UINT16 key = bioskey(_KEYBRD_READ);
		key = ((__translate_table[ ((key >> 8) & 0x00FF) ] << 8) & 0xFF00) | (key & 0xFF);
		cwt_post_kbd_event(key, bioskey(_KEYBRD_SHIFTSTATUS));
	}
}

void cwt_post_kbd_event(UINT16 key, UINT16 mods)
{
	CWT_KBD_EVENT *evt = JQ_MALLOC(CWT_KBD_EVENT);
	...
	evt->mods &= ~0x200;
	cwt_post_event(CWT_EVT_KEYBOARD, evt, CWT_DEFAULT_FREE);
}
