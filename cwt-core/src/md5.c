...
#if CWT_BYTE_ORDER == CWT_BIG_ENDIAN
...
/*--------------------------------------------------------------------------*/
void md5_update(md5_state_t *pms, const md5_byte_t *data, int nbytes)
{
...
char *md5_str(char *p)
{
	...
	/* hashage */
	md5_init(&state);
	md5_update(&state, (const md5_byte_t *)p, (int)strlen(p));
...
char *md5_file(FILE *fp)
{
	...
	while(!feof(fp)) {
		...
		md5_update(&state, (const md5_byte_t *)data, (int)ln);
	}
	md5_finish(&state, digest);
