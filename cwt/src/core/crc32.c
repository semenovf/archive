...
static INT32 __crc32_lookup_table[] = {
...
INT32 __calculateCRC32(const void *pdata, size_t nbytes, INT32 crc32)
{
	const BYTE *pbytes = (const BYTE*)pdata;
	/*crc32 = 0xFFFFFFFF;*/
	crc32 = crc32 ^ 0xFFFFFFFF;
	while( nbytes-- )
		crc32 =  __crc32_lookup_table[(crc32 ^ *pbytes++) & 0xff ] ^ (((UINT32)crc32) >> 8);
	/*crc32 = ~crc32;*/
	crc32 = crc32 ^ 0xFFFFFFFF;
	return crc32;
}
...
static INT32 __crc32_poly32rev  = 0xEDB88320L;

static void __cwtCrc32GenLookupTable(INT32 tab[256])
{
	int i, j;
...
DLL_API_EXPORT void cwt_test_crc32(void)
{
	int i;
	INT32 tab[256];
	char teststr[256];

	__cwtCrc32GenLookupTable(tab);

	for( i = 0; i < 256; i++ ) {
		sprintf(teststr, "Element %d in static crc32_lookup_table[%d] == generated_table[%d]", i, i, i);
		CWT_TEST_OK2(__crc32_lookup_table[i] == tab[i], _T("teststr"));
	}
}
#endif
