...
struct COMDesc
{
  ...
  int bHiSpeed;     /* Indicates whether serial port supports high speed up to 921600L */
...
int COMPortOpen(int nCOM, long nBauds, int nWordLen, int nParity,
  int nStopBits, int nFlowControl, int bHiSpeed, void (*EventHandler)(int nEvent))
{
  struct COMDesc *pCOM;
  unsigned char	x;
  int nRXQueueSize;
  ...
  */
  COMSetTransmitParameters(nCOM, nBauds, nWordLen, nParity, nStopBits, bHiSpeed);
...
void COMSetTransmitParameters(int nCOM, long nBauds, int nWordLen,
  int nParity, int nStopBits, int bHiSpeed)
{
  struct COMDesc *pCOM;
  long nDivisor;
  int nDivLow;
...
void COMGetTransmitParameters(int nCOM, long *pBauds, int *pWordLen,
  int *pParity, int *pStopBits, int *pHiSpeed )
{
  struct COMDesc *pCOM;
