...
void LoadBean::notifyInitWalk (AV & av)
{
	...
	pfs::string query;
	query << "SELECT * FROM " << av.getScope().table()
			<< " WHERE " << HIBERLITE_PRIMARY_KEY_COLUMN << "=" << pfs::stringify(curId) << ";";

	cwt::debby::statement sel = av.getConnection().prepare(query);
...
void LoadBean::act (AV & av, collection_nvp<E,S> nvp)
{
	...
	pfs::string tab(av.getScope().table());
	...
	for (size_t i = 0; i < children.size(); ++i) {
		...
		query << "SELECT * FROM " << tab
		      << " WHERE " << HIBERLITE_PRIMARY_KEY_COLUMN << "=" << pfs::stringify(children[i]) << ";";
