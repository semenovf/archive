#ifndef __HIBERLITE_BEANUPDATER_H__
#define __HIBERLITE_BEANUPDATER_H__
...
struct RowScope
{
	...
	template<class C>
	inline void addAssignment (const pfs::string & name, db_atom<C> & atom);

	void addSimpleAssign (const pfs::string & name, const pfs::string & value);

	~RowScope();
};

// TODO Refuse this class
#ifdef __COMMENT__
class Transformer
{
	public:
...
protected:
	...
	void startRow (const pfs::string & table, sqlid_t rowid, sqlid_t parent_id, sqlid_t index);
	void commitRow (cwt::debby::database & con, sqlid_t rowid);
};
...
#endif // __HIBERLITE_BEANUPDATER_H__
