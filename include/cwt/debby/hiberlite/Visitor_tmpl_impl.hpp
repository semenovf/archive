...
template<class C>
cwt::debby::database AVisitor<C>::getConnection()
{
	return rootKey.con;
}
...
void AVisitor<C>::startWalk (Y & obj, bean_key key)
{
	...
	pfs::string ClassName = Database::getClassName<Y>();
...
template<class Vis>
void AVisitor<Vis>::divePrefix (const pfs::string & name)
{
	pushAndSubPrefix(name);
}

template<class Vis>
void AVisitor<Vis>::diveTable (const pfs::string & name)
{
...
template<class C>
void AVisitor<C>::pushAndSubTable(const pfs::string & name)
{
	stack.push(scope);
...
template<class C>
void AVisitor<C>::pushAndSubPrefix (const pfs::string & prefix)
{
	stack.push(scope);
