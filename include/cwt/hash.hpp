...
inline uint qHash(const String &key)    { return qHash(*reinterpret_cast<const QString*>(key.__impl)); }
inline uint qHash(const ByteArray &key) { return qHash(*reinterpret_cast<const QByteArray*>(key.__impl)); }
