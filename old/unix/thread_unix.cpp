...
bool thread_impl::setStackSize (pthread_attr_t & attr, size_t stackSize)
{
    ...
    if (stackSize > 0) {
    	...
    	rc = PFS_VERIFY_ERRNO(pthread_attr_setstacksize(& attr, stackSize));
    }

    _stackSize = 0;
    rc = PFS_VERIFY_ERRNO(pthread_attr_getstacksize(& attr, & _stackSize));
...
void thread_impl::start (thread::priority_type priority, size_t stackSize)
{
	...
	do {
		rc = PFS_VERIFY_ERRNO(pthread_attr_init(& attr));
		if (rc)
			break;

		// Only threads that are created as joinable can be joined.
		// If a thread is created as detached, it can never be joined.
		rc = PFS_VERIFY_ERRNO(pthread_attr_setdetachstate(& attr, PTHREAD_CREATE_DETACHED));
		if (rc)
			break;
		...
		if (priority == thread::InheritPriority) {
			rc = PFS_VERIFY_ERRNO(pthread_attr_setinheritsched(& attr, PTHREAD_INHERIT_SCHED));
			if (rc)
				break;
		} else {
...
#ifdef PFS_HAVE_THREAD_PRIORITY_SCHEDULING
			int posixPolicy;
			rc = PFS_VERIFY_ERRNO(pthread_attr_getschedpolicy(& attr, & posixPolicy));
            if (rc) {
	...
	if (rc == 0) {
		do {
			...
			_data->threadImpl = _thread->base_class::cast(); //_thread->_pimpl;

			rc = PFS_VERIFY_ERRNO(pthread_create(& pth, & attr, & thread_impl::thread_routine, this));
...
void * thread_impl::thread_routine (void * arg)
{
    ...
    PFS_VERIFY_ERRNO(pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, nullptr));
    ...
    PFS_VERIFY_ERRNO(pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, nullptr));
    PFS_VERIFY_ERRNO(pthread_setcanceltype(PTHREAD_CANCEL_DEFERRED, nullptr));
...
void thread_impl::terminate ()
{
    ...
    if (_data && _data->threadId) {
    	...
    	PFS_VERIFY_ERRNO(pthread_cancel(_data->threadId)); // thread termination error
    }
}
...
void thread_impl::setPriority(thread::priority_type priority)
{
    ...
    int rc = PFS_VERIFY_ERRNO(pthread_getschedparam(_data->threadId, & posixPolicy, & param));
    if (rc)
        return;
    ...
    rc = PFS_VERIFY_ERRNO(pthread_setschedparam(_data->threadId, posixPolicy, & param));
