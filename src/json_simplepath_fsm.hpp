...
bool _is_root(const void *data, size_t len, void *context, void *action_args)
{
	...
	if (len > 0) {
		...
		ctx->m_jsonValue = &ctx->m_jsonPath->json().value();
	}
#else
...
bool _on_end_elem(const void *data, size_t len, void *context, void *action_args)
{
	...
	if (!ctx->m_key.isEmpty()) {
		ctx->m_jsonValue = &ctx->m_jsonValue->at(ctx->m_key);
	}
	...
	if (!ctx->m_subscript.isEmpty()) {
		...
		if (ok)
			ctx->m_jsonValue = &ctx->m_jsonValue->at(index);
		else
			ctx->m_jsonValue = &JsonValue::sharedNull;
	}

	if (ctx->m_jsonValue == &JsonValue::sharedNull)
		return false;
	return true;
