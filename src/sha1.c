 ...
 */
static INLINE sha1_word_t packup(const sha1_byte_t *p)
{
	/* Portable, but slow */
 ...
 * Processing a block
 */
static inline void sha1_update_now(sha1_state_t *pms, const sha1_byte_t *bp)
{
 ...
 */
void sha1_update(sha1_state_t *pms, const sha1_byte_t *bufp, int length)
{
	/* Is the buffer partially filled? */
	if(pms->sha1_count != 0) {
		if (pms->sha1_count + length >= (int)sizeof(pms->sha1_buf)) {	/* buffer is filled enough */
	...
	for(;;) {
		if(length < (int)sizeof(pms->sha1_buf)) {		/* Short to fill up the buffer */
			if(length) {
...
void sha1_finish(sha1_state_t *pms, sha1_byte_t output[SHA1_OUTPUT_SIZE])
{
	...
	/* fill zeros */
	if(pms->sha1_count > (int)(sizeof(pms->sha1_buf) - 2 * sizeof(sha1_word_t))) {
	...
	for(i = 0; i < (int)(sizeof(pms->sha1_h)/sizeof(sha1_word_t)); i++) {
...
char *sha1_str(char *p)
{
	...
	char *hex_output = (char *)malloc(SHA1_OUTPUT_SIZE * 2 + 1);
	int di;

	/* hashage */
	sha1_init(&state);
	sha1_update(&state, (const sha1_byte_t *)p, (int)strlen(p));
	sha1_finish(&state, digest);

	/* output */
	for (di = 0; di < SHA1_OUTPUT_SIZE; di++) {
		sprintf(hex_output + di * 2, "%02x", digest[di]);
	}
	return hex_output;
}

char *sha1_file(FILE *fp)
{
	sha1_state_t state;
	sha1_byte_t digest[SHA1_OUTPUT_SIZE];
	char data[64];
	...
	while(!feof(fp)) {
		...
		sha1_update(&state, (const sha1_byte_t *)data, (int)ln);
	}
	sha1_finish(&state, digest);
