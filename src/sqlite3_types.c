...
CWT_CHAR* sqlite3_stringify_int_type(LONGLONG min, ULONGLONG max)
{
	CWT_UNUSED2(min, max);
	return cwt_str_ns()->strDup(_T("INTEGER"));
}

CWT_CHAR* sqlite3_stringify_float_type(CwtTypeEnum type, UINT prec, UINT scale)
{
	CWT_UNUSED3(type, prec, scale);
	return cwt_str_ns()->strDup(_T("REAL"));
}

CWT_CHAR* sqlite3_stringify_text_type(ULONGLONG maxlen)
{
	CWT_UNUSED(maxlen);
