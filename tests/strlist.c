...
int main(int argc, char *argv[])
{
	...
	CWT_TEST_OK(strNS->eq(_T("Lorem"), slNS->at(sl, 0)));
	CWT_TEST_OK(strNS->eq(_T("ullamcorper"), slNS->at(sl, 31)));
	CWT_TEST_OK(strNS->eq(_T("futurum."), slNS->at(sl, 171)));

	slNS->clear(sl);
	CWT_TEST_OK(slNS->size(sl) == 0);
	CWT_TEST_OK(slNS->splitAny(sl, loremipsum_doublequoted, _T(" "), CWT_QUOTES_DOUBLE) == 35);
	CWT_TEST_OK(strNS->eq(_T("Lorem"), slNS->at(sl, 0)));
	CWT_TEST_OK(strNS->eq(_T("Eodem"), slNS->at(sl, 22)));
